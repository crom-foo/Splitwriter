...
type FolderNavState = { files: string[]; index: number };

function isImageFilePath(p?: string | null) {
  return !!p && IMAGE_EXT_RE.test(p.split(/[?#]/)[0] || "");
}

function isLocalFsPath(p?: string | null) {
  if (!p) return false;
  return /^[A-Za-z]:[\\/]/.test(p) || p.startsWith("/") || p.startsWith("\\\\");
}

function sameFsPath(a?: string | null, b?: string | null) {
...
function pathForFolderNav(raw?: string | null) {
  const p = normalizePath(raw || "");
  return isLocalFsPath(p) ? p : "";
}
...
const ImageBoard = forwardRef<ImageBoardHandle, Props>(function ImageBoard(
  ...
  const [folderNav, setFolderNav] = useState<FolderNavState>({ files: [], index: -1 });
  const canFolderNav = folderNav.files.length > 1;
  ...
  useEffect(() => {
    ...
    (async () => {
      const currentPath = pathForFolderNav(displayPath ?? src ?? resolved.missing ?? null);
      if (!isTauri() || !currentPath) {
        if (alive) setFolderNav({ files: [], index: -1 });
        return;
      }

      try {
        const [{ readDir }, { dirname }] = await Promise.all([
        ...
        const files = entries
          .map((e: any) => e?.path as string | undefined)
          .filter((p): p is string => !!p && isImageFilePath(p))
          .sort((a, b) => baseName(a).localeCompare(baseName(b), undefined, { numeric: true, sensitivity: "base" }));

        const index = files.findIndex((p) => sameFsPath(p, currentPath));
        if (alive) setFolderNav({ files, index });
      } catch (err) {
        if (IS_DEV) console.warn("[ImageBoard] folder navigation scan failed", err);
        if (alive) setFolderNav({ files: [], index: -1 });
      }
    })();

    return () => { alive = false; };
  }, [displayPath, src, resolved.missing]);
  ...
  function navigateFolderImage(step: -1 | 1) {
    if (!canFolderNav) return;

    const len = folderNav.files.length;
    const baseIndex = folderNav.index >= 0 ? folderNav.index : (step > 0 ? -1 : 0);
    const nextIndex = (baseIndex + step + len) % len;
    const nextPath = folderNav.files[nextIndex];
    if (!nextPath || sameFsPath(nextPath, pathStr)) return;

    const imageId = wrapRef.current?.getAttribute("data-image-id") || undefined;
    window.dispatchEvent(
      new CustomEvent("sw:image:use", {
        detail: { path: nextPath, imageId },
      })
    );
  }
  ...
  return (
    ...
    >
      {canFolderNav && !!src && (
        <div
          title={`${baseName(folderNav.files[Math.max(0, folderNav.index)] || pathStr)} (${Math.max(0, folderNav.index) + 1}/${folderNav.files.length})`}
          style={{
            position: "absolute",
            top: 6,
            right: 8,
          ...
          onMouseEnter={(e) => {
            ...
            el.style.background = "rgba(10,12,16,0.58)";
            el.style.borderColor = "rgba(255,255,255,0.28)";
            el.style.boxShadow = "0 6px 18px rgba(0,0,0,0.26)";
          }}
          onMouseLeave={(e) => {
            const el = e.currentTarget;
            el.style.opacity = "0.42";
            el.style.background = "rgba(10,12,16,0.34)";
        ...
        >
          <NavBtn label="Previous image" onClick={() => navigateFolderImage(-1)}>◀</NavBtn>
          <NavBtn label="Next image" onClick={() => navigateFolderImage(1)}>▶</NavBtn>
        </div>
      )}
  ...
  return (
    <button
      ...
      style={{
        ...
        color: hover ? "rgba(255,255,255,0.96)" : "rgba(231,234,238,0.78)",
        fontSize: 10,
        lineHeight: 1,
        cursor: "pointer",
        userSelect: "none",
